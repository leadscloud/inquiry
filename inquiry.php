	...
	case 'view':
		...
		*/
		...
		if(current_user_can('inquiry-delete',false)){
			echo   '<h2 id="d_clip_container">详细信息<a class="button" href="'.preg_replace('/\s\s+/',null,$referer.'?'.$query_url).'">返回</a> <a class="button" href="inquiry.php?postid='.$data['id'].'&method=delete&nextid='.$nextpostid['id'].'">删除</a> <a class="button" id="d_clip_button" href="javascript:;">一键复制</a></h2>';
		}else{
		...
		echo   '<fieldset'.($data['from_company']=='CPM'?' class=\"from_compnayname\"':' class="from_compnayname"').'>';
		echo       '<table class="form-table">';
		echo			'<thead class="infos">';
		echo               '<tr>';
		//echo                   '<td><label>一般信息 </td>';
		echo                   '<td colspan="2"><span>来源 IP: <a target="_blank" title="点击查看详细信息" href="http://www.123cha.com/ip/?q='.$fromip.'">'.$fromip.'</a> (检测结果：'.ip2addr($fromip).'  )</span><span>所属国家:'.$country.'</span><span>时间:'.$data['time'].'</span><span>来源网站:'.$data['website'].'</span></td>';
		echo               '</tr>';
		echo			'</thead>';
		echo           '<tbody>';
			...
			echo           '</tr>';
		echo               '<tr>';
		echo                   '<th><label>姓名 <span>(Name)</span></th>';
		echo                   '<td>';
		echo                       $data['name'];
		echo                   '</td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>邮件 <span>(Email)</span></th>';
		echo                   '<td>'.$data['email'].'</td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>内容 <span>(Contents)</span></th>';
		echo                   '<td id="inquiryContent"><div id="txtString">'.$inquiry_content.'</div></td>';
		//$tran = Google_translate_API();
		echo               '</tr>';
		echo               '<tr>';
		echo 					'<th><select id ="txtLang" name="txtLang"><option value="zh-CHS">zh-CHS</option><option value="zh-CHT">zh-CHT</option></select><a class="black pl10" href="#" id="getdata-button">翻译</a></th>';
		...
		echo                   '<td id="TranslateResult"><div class="bgwhite width500" id="showdata"></div></td>';
		echo 					'';
		echo               '</tr>';
		echo               '<tr>';
  //echo $res;
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>电话 <span>(Phone)</span></th>';
		echo                   '<td id="phone_num">'.$data['phone'].'</td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>地址 <span>(Address)</span></th>';
		echo                   '<td>'.$data['address'].'</td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>来源 <span>(From)</span></th>';
		echo                   '<td><a href="'.$data['website'].'" target="_blank" title="新窗口打开网站">'.$data['website'].'</a></td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><label>网站所属品牌</th>';
		echo                   '<td>'.($data['from_company']==NULL?"西芝":$data['from_company']).'</td>';
		echo               '</tr>';
		echo               '<tr>';
		echo                   '<th><div id="d_clip_container2" style="position:relative">
   <div id="d_clip_button2" class="button-secondary">一键复制</div>
  </div></th>';
		echo                   '<td></td>';
		echo               '</tr>';
		echo           '</tbody>';
	...
	//header('Content-Type: text/html; charset=UTF-8');
		...
		$end_datetime   = isset($_REQUEST['end_datetime'])?$_REQUEST['end_datetime']:'';
		//$query    = array('page' => '$');
		...
		if ($datetime||$end_datetime) {
			...
			$where = "WHERE `time` > '".date('Y-m-d H:i:s',$datetime)."' AND `time` < '".date('Y-m-d H:i:s',$end_datetime)."'";
		}
		//echo $where;
		...
		if($datetime)
			$sql="select * from `#@_inquiry` {$where} order by `time` ASC";
		else
		...
		//echo PHP_FILE;
		if($read=='0'||$read=='1') 
			//$page_url   = PHP_FILE.'?type=inquiry&'.$_SERVER['QUERY_STRING'].'&'.http_build_query($query);
			//$page_url = rtrim($_SERVER["SCRIPT_NAME"]."?".$_SERVER['QUERY_STRING'],'/').'&'.http_build_query($query);
        ...
        if ($result) {
            while ($data = pages_fetch($result)) {
				...
				$fromsite = mb_substr(clear_space(strip_tags($post['website'])),0,60,'UTF-8');
				//print_r($post);
				//echo $data['id'];
				...
				echo    '<td><div title="'.date('Y-m-d H:i:s',strtotime($post['time'])).'">'.date('Y-m-d',strtotime($post['time'])).'</div></td>';//'.get_icon($post['approved']).'
                echo '</tr>';
            }
        } else {
